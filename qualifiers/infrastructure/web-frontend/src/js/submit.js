...
function watchExploit(exploit) {
  ...
  function updateExploit() {
    fetch(API_ROOT + "/exploits/" + exploit.exploit_id, {
      ...
      if (response.status === "RunSolved" || response.status === "RunFailed") {
        let $outputButton = $("<a>").attr("href", "javascript:void(0);").text("Output");
        $li.append("&nbsp;");
        $li.append($outputButton);

        let $outputBox = $("<div>").addClass("output-box");
        $outputBox.hide();
        $li.append($outputBox);

        let $stdoutBox = $("<pre>");
        let $stderrBox = $("<pre>");
        $outputBox.append($stdoutBox);
        $outputBox.append($stderrBox);
        ...
        $outputButton.click(() => {
          $outputBox.toggle();
          if (needDownload) {
            fetch(API_ROOT + "/exploits/" + exploit.exploit_id + "/output", {
              headers: headers
            }).then((response) => {
              return response.json().then((json) => {
                return {
