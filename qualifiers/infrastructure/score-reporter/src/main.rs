...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for challenge in all_challenges.iter() {
        ...
        let challenge_scores =
            common::get_challenge_scores(&mut conn, challenge, &scoring_parameters)
                .await
        ...
        for solve in challenge_scores.solves.iter() {
            ...
            log::info!(
                "Going to submit {solve_score} points to team {} for explioit {solve_exploit_id}",
                solve.team_id
            );
            ...
            match report_response {
                ...
                NautilusScoreResponse::Score {
...

                    {
                        ...
                        {
                            log::warn!(
                                "Failed to store score in database for exploit {solve_exploit_id}: {err}"
                            );
                            continue;
                        }
                    }
